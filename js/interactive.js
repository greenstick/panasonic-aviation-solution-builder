...
// Next Slide
Interactive.prototype.next = function (slide, count, callback) {
...
//An Optional Limit (Int) Parameter Can Be Passed To Specify The Maximum Number of Tiles That Can Be Selected.
Interactive.prototype.select = function (id, parent, limit, cache) {
	var element = $('#' + id);
	...
	if (limit && typeof limit === 'number') {
		...
		else if ($('#' + parent + ' .selected').length > limit - 1) {
		...
		//else add selected and check to see if limit rule has been reached, if so render other buttons inactive
			...
			if ($('#' + parent + ' .selected').length > limit - 1) {
				$('#' + parent + ' .button:not(.selected)').addClass('inactive');
			};
		};
	}
...
//Gathers Selected Elements on Specified Slide (Str) and Places Them in Specified Array (array) 
//Allows for Optional Parent (Bool) The Flags Parent ID to Be Pushed into Array
...
//Matches Selected Features Array (array) to Their Specific Addons Specified in Data (JSON)
//Returns New Array of Addons
...
/*
Combined Navigation & Selection Processing Event Bindings - Next Slide
...
$('#s2 .next, #s2 .toBackground').on("click", function (e) {
	//Prevent Next Until Select Completed
	if ($('#s2 .selected').length < 2) {
		return;
	};
...
/*
Navigation Event Bindings - Previous Slide
...
/*
User Select Event Bindings - Slide Specific
*/
...
$('#s3 .input').on("click", function (e) {
	interactive.select(e.currentTarget.id, "s3", null, interactive.s3Inactive);
});
